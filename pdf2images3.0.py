...
def pyMuPDF_fitz(pdfPath, imagePath, imageName):
    ...
    for pg in range(pdfDoc.pageCount):
        ...
        fontOne = ImageFont.truetype("‪C:\Windows\Fonts\simfang.ttf", 20)  # 本地字体文件
        ...
        draw.text((imageInfo.size[0] // 2 - 25, imageInfo.size[1] // 2 + 445), u"第%s页"%str(int(pg) + 1), fill=(0, 0, 0), font=fontOne)
        ...
        imageInfo.save(imagePath + '/' + imageName + '-%s.JPEG' % str(int(pg) + 1), quality=1000, optimize=True, progressive=True)
...
if __name__ == "__main__":
    ...
    Label(root, text="请输入科目代码：").place(relx=0.45, rely=0.3, anchor=CENTER)
    ...
    def Info():
        ...
        sub_code = e3.get()  # 日期
        sub_code = sub_code.strip()  # 去除空格
        sub_code = sub_code.replace(" ", "")  # 去除换行符
        sub_code = sub_code.replace("\n", "")  # 去除换行符


        pdfPath = path.get()    # 文件路径

        print('学校:' + sch_name + ',' + '学校代码:' + sch_code + ',' + '真题年份:' + sub_code + ',' + '文件路径:' + pdfPath)
        ...
        #date = info[2]
        # 定义excel表名
        workbook = xlsxwriter.Workbook(sch_name + sub_code + '真题信息表' + '.xlsx')
        # 创建一个空表
        ...
        for file in file_list:
            ...
            worksheet.write(count, 4, '含答案详解;pdf;纸质版;内容完整')
        ...
        for file in file_list:
            ...
            imagePath = pdfPath + '\\' + sch_code + '\\' + sub_code + '\\' + date

            # 以学校代码-年份-学科代码.png命名图片
            imageName = sch_code + '-' + sub_code + '-' + date
